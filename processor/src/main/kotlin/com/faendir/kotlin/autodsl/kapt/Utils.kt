...
fun TypeName.mapToKotlin(): TypeName =
    when (this) {
        is ClassName -> {
            mapToKotlin()
        }
        ...
        is ParameterizedTypeName -> {
            rawType.mapToKotlin().parameterizedBy(typeArguments.map { it.mapToKotlin() })
        }

        is WildcardTypeName -> {
            if (inTypes.isEmpty()) {
                WildcardTypeName.producerOf(outTypes.first().mapToKotlin())
            } else {
